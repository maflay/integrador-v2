...
const totalPairs = cells.length / 6; // 12 celdas => 6 parejas
...
// Función para contar cuántas parejas válidas existen tomando en cuenta TODAS las fichas actualmente volteadas
function evaluateRevealedPairs() {
  ...
  for (const key in keysCount) {
    // Cada par completado de la misma key suma 1 pareja (ej. 2 fichas de la misma key = 1 pareja)
...
function onCellClick(e) {
  ...
  // Validación estricta del límite de 4 movimientos antes de permitir voltear otra
  if (moves >= 4) {
    Swal.fire({
      icon: "warning",
  ...
  // Recalcular parejas basándose en todas las fichas reveladas hasta el momento (sin importar orden de elección)
  ...
  // Comprobar si ya se consumieron los 4 movimientos tras la acción
  if (moves >= 4) {
...
function ensureMarkup() {
  document.querySelectorAll(".cell-c").forEach((cell, index) => {
    if (!cell.querySelector(".card-c")) {
      // Usamos index + 1 para que vaya de 1 hasta el total de fichas (cells.length)
      cell.innerHTML = `
          <div class="card-c">
...
function deal() {
  ...
  document.querySelectorAll(".cell-c").forEach((cell, i) => {
    ...
    if (frontNum) {
      ...
      // O si prefieres mostrar formato tipo "1 / 12":
      // frontNum.innerHTML = `<span class="card-number">${i + 1} de ${cells.length}</span>`;
    }
